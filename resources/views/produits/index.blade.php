...
@section('content')
    ...
    <h2 class="text-3xl font-bold text-gray-900 mb-6 border-b-4 border-red-600 pb-2 flex items-center">
        <i class="fa-solid fa-vials text-red-600 mr-3 text-2xl hidden md:inline"></i>
    ...
    @if ($produits->isEmpty())
        <div class="bg-white rounded-xl border-2 border-dashed border-gray-300 p-12 text-center">
            ...
            <p class="text-gray-500 text-lg font-medium">Aucun produit enregistré pour le moment.</p>
        </div>
    @else
        {{-- Formulaire de recherche --}}
        <form method="GET" action="{{ route('produits.index') }}" class="mb-6 flex flex-col sm:flex-row gap-3">
            <label class="input inline-flex items-center border border-gray-300 rounded-lg px-3 py-2 bg-white focus-within:ring-2 focus-within:ring-red-500 focus-within:border-red-500 transition-all flex-1 max-w-md">
                ...
                <input name="search" type="search" placeholder="Rechercher un produit..." value="{{ request('search') }}"
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @foreach ($produits as $produit)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3 border text-sm text-gray-700">{{ $produit->codeproduit }}</td>
                            <td class="px-4 py-3 border text-sm font-medium text-gray-900">{{ $produit->libelle }}</td>
                            <td class="px-4 py-3 border text-sm text-gray-700">{{ $produit->conditionnement }}</td>
                            <td class="px-4 py-3 border text-sm text-gray-700">{{ $produit->quantitestock }}</td>
                            <td class="px-4 py-3 border text-sm text-gray-700">{{ $produit->stockmax }}</td>
                            <td class="px-4 py-3 border text-sm text-gray-700">{{ $produit->stockmin }}</td>
                            <td class="px-4 py-3 border text-sm text-gray-700">{{ $produit->stocksecurite }}</td>
                            <td class="px-4 py-3 border text-sm text-gray-700">{{ $produit->dateperemption }}</td>
                            <td class="px-4 py-3 border text-sm text-gray-700">{{ $produit->lot }}</td>
                            <td class="px-4 py-3 border text-sm text-gray-700">{{ \Carbon\Carbon::parse($produit->date)->translatedFormat('d F Y') }}</td>
                ...
                @for ($page = 1; $page <= $produits->lastPage(); $page++)
                    <label class="relative">
                        ...
                        <span class="px-4 py-2.5 text-sm font-medium transition-all duration-200 cursor-pointer
                                    ...
                                    @endif
                                    @if($page < $produits->lastPage()) border-r border-gray-200 @endif">
                            {{ $page }}
                        </span>
                    </label>
                @endfor
