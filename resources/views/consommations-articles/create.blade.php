...
@if (!$peutVoir)
    <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg max-w-2xl mx-auto mb-8">
        <div class="flex items-start">
            ...
            <p class="text-red-800 font-medium">⚠️ Vous n'êtes pas autorisé à créer ou modifier une fiche de consommation.</p>
        </div>
    </div>
@endif
        ...
        <table class="min-w-full border border-gray-200 rounded-xl shadow-sm">
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                @foreach ($consommations as $c)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 border text-sm font-medium text-gray-900">{{ $c->article->libelle ?? 'N/A' }}</td>
                            ...
                            @if ($peutModifier)
                                <form id="delete-form-{{ $c->consommationArt_id }}"
                                    ...
                                    <button type="button" onclick="confirmDelete('{{ $c->consommationArt_id }}')"
                                        class="text-red-600 hover:text-red-800 transition-colors" title="Supprimer" aria-label="Supprimer">
