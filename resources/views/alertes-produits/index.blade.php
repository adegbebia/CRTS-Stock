...
@section('title', 'Liste des alertes produits')
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @foreach($alertesProduits as $alerte)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3 border text-sm font-medium text-gray-900">{{ $alerte->produit->libelle ?? 'Produit inconnu' }}</td>
                            ...
                            <td class="px-4 py-3 border text-sm">
                                <a href="{{ route('alertes-produits.show', $alerte) }}" class="text-blue-600 hover:text-blue-800 transition-colors" title="Voir les détails">
                                    <button type="button">
                ...
                @for ($page = 1; $page <= $alertesProduits->lastPage(); $page++)
                    <label class="relative">
                        ...
                        <span class="px-4 py-2.5 text-sm font-medium transition-all duration-200 cursor-pointer
                                    @if($alertesProduits->currentPage() == $page)
                                        bg-red-600 text-white
                                    @else
                                        bg-white text-gray-700 hover:bg-gray-50 hover:text-red-600
                                    @endif
                                    @if($page < $alertesProduits->lastPage()) border-r border-gray-200 @endif">
                            {{ $page }}
                        </span>
                    </label>
                @endfor
