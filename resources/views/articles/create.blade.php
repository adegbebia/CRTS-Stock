...
@else
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-gray-200 p-8">
        <div class="flex items-center mb-8 pb-4 border-b border-gray-200">
            <i class="fa-solid fa-utensils text-red-600 text-3xl mr-4"></i>
            <h2 class="text-3xl font-bold text-gray-900">Ajouter un nouvel article</h2>
        </div>
        ...
        <form action="{{ route('articles.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="codearticle" id="codearticle" required pattern="^[^,;:\?!=%.@&()$*#^{}<>+/\s]+$"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="lot" id="lot" pattern="^[^,;:\.?!=%@&()$*#^{}<>+/\s]+$"
                       title="Ne doit pas contenir les caractères , ; : @ & ( ) $ * # ^ { } < > + ou espaces" value="{{ old('lot') }}" 
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all" />
                    @error('lot')
                        <p class="text-red-600 mt-1 text-sm flex items-center">
                            <i class="fa-solid fa-circle-exclamation mr-1 text-xs"></i>
