...
<form action="{{ $peutModifier ? route('mouvements-articles.store') : '#' }}" method="POST"
    class="max-w-5xl mx-auto bg-white rounded-2xl shadow-xl border border-gray-200 p-8 space-y-6"
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            @if ($articlesSelectionne)
                ...
                <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg">
                    <p class="text-gray-800 font-semibold flex items-center">
                        <i class="fa-solid fa-bowl-food text-blue-600 mr-2"></i>
                        Article sélectionné :
                        <span class="text-red-600 font-bold ml-1">{{ $article->libelle ?? 'N/A' }}</span>
                    </p>
        ...
        <div class="md:col-span-3">
            ...
            <textarea name="observation" id="observation"
                pattern="^[^,;:\.?!=%@&()$*#^{}<>+\/]+$"
                title="Ne doit pas contenir les caractères , ; : @ & ( ) $ * # ^ { } < > + /"
                class="mt-1 block w-full rounded-lg border border-gray-300 px-4 py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 resize-y min-h-[100px] transition-all"
...
<form method="GET" action="{{ route('mouvements-articles.create') }}"
    ...
    <div class="flex-1 min-w-[200px]">
        <label for="article" class="block text-sm font-medium text-gray-700 mb-1">Filtrer par article :</label>
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                @foreach ($mouvements as $mouvement)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 border text-sm font-medium text-gray-900">{{ $mouvement->article->libelle ?? 'N/A' }}</td>
