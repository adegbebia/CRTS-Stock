...
class RapportProduitController extends Controller
{
    ...
    public function genererRapportLatex($periodeType, $periode, $annee)
    {
        ...
        $fileName = "rapport-{$periodeType}-{$periode}-{$annee}-produits";
        ...
        // IMPORTANT : kpathsea (TeX Live) a besoin d'un chemin ABSOLU dans
        // argv[0] pour se localiser lui-meme et trouver ses paquets. Un
        // simple 'pdflatex' resolu via $PATH ne suffit pas (erreur
        // "Can't get directory of program name"). On resout donc le
        // chemin reel via `which`, avec repli sur /usr/bin/pdflatex.
        $pdflatexPath = config('services.pdflatex_path')
            ?: (trim((string) shell_exec('which pdflatex 2>/dev/null')) ?: '/usr/bin/pdflatex');
        $process = new Process([
            $pdflatexPath,
        ...
        // Le repertoire de travail doit etre celui des rapports : c'est la
        // que pdflatex cherche les images referencees en relatif
        // (\includegraphics{logo_crts.png}, etc.), pas la racine du projet.
        $process->setWorkingDirectory(storage_path('app/rapports'));
        $process->run();

        $pdfPath = storage_path("app/rapports/{$fileName}.pdf");

        // pdflatex peut retourner un code d'erreur meme pour de simples
        // avertissements (image manquante, etc.) alors que le PDF est
        // quand meme genere. On ne bloque donc que si AUCUN PDF n'existe.
        if (!file_exists($pdfPath)) {
            return response(
                "Erreur pdflatex :\n" .
                "--- Sortie standard (erreurs LaTeX) ---\n" . $process->getOutput() . "\n" .
                "--- Erreur système (si applicable) ---\n" . $process->getErrorOutput(),
                500
            );
        }
